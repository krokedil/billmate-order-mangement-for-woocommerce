 ...
 * Activate payment request class
 */
class BOM_Request_Activate_Payment extends BCO_Request {
	...
	public function request( $bco_order_number ) {
		...
		$request_args = apply_filters( 'bco_activate_payment_args', $this->get_request_args( $bco_order_number ) );
		...
		// Log the request.
		$log = BCO_Logger::format_log( $bco_order_number, 'GET', 'BCO activate payment', $request_args, json_decode( wp_remote_retrieve_body( $response ), true ), $code );
		BCO_Logger::log( $log );
