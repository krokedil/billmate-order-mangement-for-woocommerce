...
class BOM_Request_Cancel_Payment extends BOM_Request {
	...
	public function request( $bco_transaction_id ) {
		...
		$request_args = apply_filters( 'bco_cancel_payment_args', $this->get_request_args( $bco_transaction_id ) );

		$response = wp_remote_request( $request_url, $request_args );
		$code     = wp_remote_retrieve_response_code( $response );

		// Log the request.
		$log = BCO_Logger::format_log( $bco_transaction_id, 'GET', 'BCO cancel payment', $request_args, json_decode( wp_remote_retrieve_body( $response ), true ), $code );
		BCO_Logger::log( $log );
