...
/**
 *
 * Test_BCO_Refund_Data_Articles_Helper class
 *
 * @package category
 */
 ...
 * Test_BCO_Refund_Data_Articles_Helper class
 */
class Test_BCO_Refund_Data_Articles_Helper extends AKrokedil_Unit_Test_Case {
	...
	/**
	 * Test BCO_Refund_Data_Articles_Helper::get_article_number
	 *
	 * @return void
	 */
	public function test_get_article_number_with_sku() {
		...
		foreach ( $this->order->get_items() as $item ) {
			...
			$product->save();
			$article_number = ( new BCO_Refund_Data_Articles_Helper() )->get_article_number( $item );
	...
	/**
	 * Test BCO_Refund_Data_Articles_Helper::get_title
	 *
	 * @return void
	 */
	public function test_get_title() {
		$this->create_order( '25' );
		foreach ( $this->order->get_items() as $item ) {
			$title = ( new BCO_Refund_Data_Articles_Helper() )->get_title( $item );
	...
	/**
	 * Test BCO_Refund_Data_Articles_Helper::get_quantity
	 *
	 * @return void
	 */
	public function test_get_quantity() {
		$this->create_order( '25' );
		foreach ( $this->order->get_items() as $item ) {
			$quantity = ( new BCO_Refund_Data_Articles_Helper() )->get_quantity( $item );
	...
	/**
	 * Test BCO_Refund_Data_Articles_Helper::get_article_price
	 *
	 * @return void
	 */
	public function test_get_article_price() {
		...
		$this->create_order( '25' );
		foreach ( $this->order->get_items() as $item ) {
			$this->assertEquals( 8000, ( new BCO_Refund_Data_Articles_Helper() )->get_article_price( $item ) );
		}
		wp_delete_post( $this->order->get_id() );
		$this->order = null;
		...
		foreach ( $this->order->get_items() as $item ) {
			$this->assertEquals( 8929, ( new BCO_Refund_Data_Articles_Helper() )->get_article_price( $item ) );
		}
		wp_delete_post( $this->order->get_id() );
		$this->order = null;
		...
		$this->create_order( '6' );
		foreach ( $this->order->get_items() as $item ) {
			$this->assertEquals( 9434, ( new BCO_Refund_Data_Articles_Helper() )->get_article_price( $item ) );
		}
		wp_delete_post( $this->order->get_id() );
		$this->order = null;
		...
		foreach ( $this->order->get_items() as $item ) {
			$this->assertEquals( 10000, ( new BCO_Refund_Data_Articles_Helper() )->get_article_price( $item ) );
		}
		wp_delete_post( $this->order->get_id() );
		$this->order = null;
	...
	/**
	 * Test BCO_Refund_Data_Articles_Helper::get_without_tax
	 *
	 * @return void
	 */
	public function test_get_without_tax() {
		$this->create_order( '25' );
		foreach ( $this->order->get_items() as $item ) {
			$this->assertEquals( 8000, ( new BCO_Refund_Data_Articles_Helper() )->get_without_tax( $item ) );
		}
		wp_delete_post( $this->order->get_id() );
		$this->order = null;
		...
		$this->create_order( '12' );
		foreach ( $this->order->get_items() as $item ) {
			$this->assertEquals( 8929, ( new BCO_Refund_Data_Articles_Helper() )->get_without_tax( $item ) );
		}
		wp_delete_post( $this->order->get_id() );
		$this->order = null;
		...
		$this->create_order( '6' );
		foreach ( $this->order->get_items() as $item ) {
			$this->assertEquals( 9434, ( new BCO_Refund_Data_Articles_Helper() )->get_without_tax( $item ) );
		}
		wp_delete_post( $this->order->get_id() );
		$this->order = null;
		...
		$this->create_order( '25', false );
		foreach ( $this->order->get_items() as $item ) {
			$this->assertEquals( 10000, ( new BCO_Refund_Data_Articles_Helper() )->get_without_tax( $item ) );
		}
		wp_delete_post( $this->order->get_id() );
		$this->order = null;
	...
	/**
	 * Test BCO_Refund_Data_Articles_Helper::get_tax_rate
	 *
	 * @return void
	 */
	public function test_get_tax_rate() {
		$this->create_order( '25' );
		foreach ( $this->order->get_items() as $item ) {
			$this->assertEquals( 25, ( new BCO_Refund_Data_Articles_Helper() )->get_tax_rate( $item ) );
		}
		wp_delete_post( $this->order->get_id() );
		$this->order = null;
		...
		$this->create_order( '12' );
		foreach ( $this->order->get_items() as $item ) {
			$this->assertEquals( 12, ( new BCO_Refund_Data_Articles_Helper() )->get_tax_rate( $item ) );
		}
		wp_delete_post( $this->order->get_id() );
		$this->order = null;
		...
		$this->create_order( '6' );
		foreach ( $this->order->get_items() as $item ) {
			$this->assertEquals( 6, ( new BCO_Refund_Data_Articles_Helper() )->get_tax_rate( $item ) );
		}
		wp_delete_post( $this->order->get_id() );
		$this->order = null;
		...
		$this->create_order( '0' );
		foreach ( $this->order->get_items() as $item ) {
			$this->assertEquals( 0, ( new BCO_Refund_Data_Articles_Helper() )->get_tax_rate( $item ) );
		}
		wp_delete_post( $this->order->get_id() );
		$this->order = null;
